...
export async function POST(request: Request) {
  try {
    ...
    // Send reset email via nodemailer using Brevo SMTP configurations
    ...
    try {
      ...
      console.error('SMTP Mailer delivery error:', mailErr);
      return NextResponse.json({ error: 'Failed to deliver reset email via SMTP relay.' }, { status: 502 });
    }
